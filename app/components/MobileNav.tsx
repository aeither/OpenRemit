...
import { CreditCard, Globe, History, Home, Settings } from "lucide-react";
  ...
  return (
    <nav className="sticky bottom-0 z-10 border-t bg-white dark:bg-neutral-900 dark:border-neutral-800">
      <div className="grid grid-cols-5">
        ...
        <Link to="/send" className="flex flex-col items-center justify-center p-3 text-muted-foreground hover:text-primary aria-[current=page]:text-primary aria-[current=page]:font-semibold">
          <CreditCard className="h-5 w-5" />
          <span className="mt-1 text-xs">Send</span>
        </Link>
        <Link to="/international" className="flex flex-col items-center justify-center p-3 text-muted-foreground hover:text-primary aria-[current=page]:text-primary aria-[current=page]:font-semibold">
          <Globe className="h-5 w-5" />
          <span className="mt-1 text-xs">Global</span>
        </Link>
